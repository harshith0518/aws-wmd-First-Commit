...
## Afternoon independent-review milestone

- Ownership checkpoint pushed as 466887c. Implemented private ISSUE service-review intake, own/assigned case list, read, attributed responses, reviewer-only notes, private history, begin/decide/require-action/close commands and eligible corrective-handler selection. One active case per requester/source; three intake requests per day and thirty responses per hour; writes use canonical guards, quota/idempotency/history/outbox transactions.
- Reviewer selection tries the configured unit escalation person, then campus independentReviewerId. Requires a current REVIEWER role and excludes requester, named subject, referenced reply author, current owner/collaborators and recorded past handlers. No candidate yields ESCALATED. An original issue handler or general administrator has no automatic access to review content. Assigned reviewers receive no automatic source-issue access.
- Added immutable INVOLVED#subject markers on publication, assignments, accepted transfers, handler replies and work commands. Reassignment cannot erase past involvement. Existing deployments with pre-marker historical data require a validated involvement backfill before enabling independent review; this workspace has synthetic data only.
- Private review events live under the review partition. Outbox jobs explicitly distinguish POST and REVIEW sources. The issue history, feed and source version do not disclose the private case. Responses/decisions recheck source-version guards to prevent concurrent handler assignment from allowing a conflicted decision. A now-ineligible reviewer cannot act/read; complainants see reviewerAvailable=false and a replacement-needed notice.
- Decisions require a reason; upheld concerns retain action/eligible owner/due date; closure after remedy retains a second attributed decision. Closure carries a fourteen-to-thirty-day appeal window (UI defaults to fifteen days). Actual appeal submission/routing is not implemented and the UI directs the user to their campus grievance contact.
- PASS: npm.cmd run check (173 operations / 241 schemas, 66 API + 3 frontend-auth tests, typechecks, all Lambda/web builds) and npm.cmd run test:integration (6 real DynamoDB scenarios). 75 tests total. The new DB case verifies concurrent review creation and competing decisions with one winner.
- Not complete: service-review files, operator assignment/reassignment, escalation/reminder jobs, actual corrective-action delivery and independent appeals. Corrective action is currently a private reviewer/requester record, not a delivered staff task. Generic review endpoints are marked partial.
- PASS: real browser journey created a private concern as the student, confirmed an empty review list for the original owner, began and decided the case as the independent reviewer, and confirmed the student sees the decision without the reviewer-only note or its history event. Browser warnings/errors: zero. Test rig stopped; its three synthetic DynamoDB tables removed. Next: remaining M3 commands/reminders and the M4 reviewed-knowledge/optional-AI slice, then deployable CDK and a truthful demo acceptance checklist. No paid cloud resources have been provisioned.

## Complete issue triage checkpoint

- Private review milestone pushed as e36ec24. Added decline (required reason/review route), duplicate (authorized lead, bounded canonical path) and set-priority (owner or lead, required reason). All fourteen issue commands now run through current authorization, versions, idempotency, atomic audit/outbox and existing guards.
- Decline/duplicate remove active queue keys, cancel temporary transfers and preserve owner, timestamps, history and original deadlines. Duplicate targets are strong-read and guarded; self/cyclic, unavailable/cross-campus targets fail. Concurrent reciprocal links have exactly one successful command. Related IDs are filtered for readers and never copied to audience-wide history. Request review remains available on declined reports.
- Real UI includes priority/review-route fields, scoped report search/radio selection and authorized related-report navigation. Browser verified High priority, duplicate creation/navigation and reasoned decline/history; zero warnings/errors. Removed an inaccurate terminal-state next-action hint after observation.
- PASS: npm.cmd run check (72 backend + 3 frontend-auth at that point), npm.cmd run test:integration (7); added one further cross-campus/inaccessible-target test and reran all 7 triage tests plus specs. Final count 83. Web typecheck/build reran after hint fix. Synthetic browser rig stopped and its exact 2e532497-2903-4dc0-89d4-dd83d4432266 tables cleaned.
- User has $100 AWS credits but explicitly deferred deployment: concentrate on making the whole system work locally. Do not deploy/provision now. Next: M4 reviewed resolution library and bounded, ACL-checked retrieval; then optional AI fallback/evaluation and remaining notification/deadline jobs. Full project still has pilot/community scope pending.

## Reviewed knowledge milestone

- Issue triage checkpoint pushed as d6467f2. Added real library create/read/search/mark-stale/review/retire APIs, shared Zod/OpenAPI contracts, curator publication form, scoped topic/team/group/keyword search, current/stale/retired cards, attributed latest review notes and source navigation. A source issue exposes its existing card so a withdrawn card remains reachable for review.
- Curators are current scoped UNIT_LEAD or REVIEWER members who can read the source. A general role does not widen the source audience. Publication requires the current reporter-confirmed resolution/version and one unique card per attempt. Reopened/replaced resolutions fail closed. Changed source versions/ACL or expired review dates make active cards effectively STALE; source/file data is not copied into a public audience.
- Source check, card, unique marker, pointers, command receipt, private card audit partition and outbox are transactional. Added conditional Delete support to the existing store so stale/retire removes exactly its discovery pointers and rereview replaces the prior review-date keys. Real DynamoDB tests verify deletion/restoration.
- Search shares a hard maximum of 100 recent discovery rows fairly across currently permitted campus/user/group partitions, deduplicates canonical cards, checks source/resolution and ranks Unicode keyword overlap plus category/unit match. An encrypted, actor/filter/auth-bound snapshot pages at most 25 results, revalidating every page/output. It is deliberately bounded rather than exhaustive; location ranking is for the subsequent precedent helper.
- PASS: full check and integration gate; 93 automated tests. The labelled search-only evaluation passed 30/30 expected results/empty outcomes. Browser PASS: curator publication, student discovery/stale flag, withdrawal from search, source-to-stale-card navigation, curator rereview and search restoration; review reasons/dates/actors visible and no browser warnings/errors. Rig stopped and exact 231de68a-8471-43d0-af2a-2b3dd58c4454 tables cleaned. No cloud model invocation, paid resources or deployment. Installed the official Bedrock SDK for the next optional adapter; dependency changes will be committed with that slice.
- Next: disabled-by-default optional precedent generation with exact preview/consent, current-source checks, citation validation, quota reservations and timeout/failure fallbacks; then the remaining deadline/notification jobs and local end-to-end acceptance.

## Deadline AWS release preparation — 20 September, evening

- User reports two hours left and now requests AWS deployment setup. This supersedes the earlier instruction to keep credits/deployment aside. Organizer's supplied email makes Bedrock optional and AWS hosting mandatory. Prioritized a deployable honest accountability-core release over unfinished R2/R3 scope; removed the unused Bedrock SDK.
- Implemented `infra/app.ts`/`stack.ts`: private S3 + CloudFront OAC/security/caching/SPA handling, Cognito Lite/classic code flow/public client/exact callbacks/no signup, JWT API with campusfix/api scope and exact CORS, Node 24 x64 API/transfer worker, three DynamoDB tables/indexes/Jobs TTL/deletion protection/retention, generated cursor secret, five-minute transfer expiry, seven-day logs and error alarms. No VPC, EC2/RDS/NAT, file pipeline, AI, SES/SQS or notification consumer.
- Added reproducible Linux Sharp Lambda packaging, CDK STS/account/bootstrap/synth/diff/deploy, frontend output-bound build/publish/invalidation, public cloud smoke, and controlled synthetic Cognito/demo seeding. Passwords only in ignored .artifacts/demo-credentials.json; accounts use reserved .example addresses, no outbound email. Six roles/identities span two hostels, handling/backup, independent reviewer and second campus. Seed creates three real workflow samples including reporter-confirmed resolution and curated knowledge; memberships expire in fourteen days.
- Found and fixed production frontend routing losing source-issue query parameters, and added validated HTTPS VITE_API_BASE_URL. Local API proxy remains the default. AWS uses separate web/API origins to avoid CloudFormation callback cycles.
- PASS: full `npm.cmd run check`: 173 operations / 241 schemas / 3 table definitions; strict application/infra types; 82 API + 5 web + 6 infra tests; all production builds. PASS offline packaged `aws:synth`; no dependency cycles. PASS Linux artifact file/ELF validation (native execution is pending Linux/CloudShell).
