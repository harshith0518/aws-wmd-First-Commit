...
- `apps/web`: React/Vite identity, reporting, drafts, scoped feed, staff queue resolution/history, evidence, threaded discussions, affected status and ownership handover screens connected to real APIs.
...
Typechecks, 65 automated tests, production builds and synthetic browser reporting/evidence checks passed. DynamoDB Local integration runs through Java because Docker is unavailable. Real Cognito/S3/GuardDuty acceptance, independent reviews, notifications, Bedrock and AWS deployment remain unverified or unfinished.
